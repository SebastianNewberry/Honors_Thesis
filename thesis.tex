...
In this conference paper by Noels, the authors thoroughly studied the censorship of AI by classifying it into soft and hard censorship \parencite{noels_llm_censorship}:
...
The experiment in this report provides a unique comparison to the results in the CyberMetric report because as of today, closed and open source models are not only better than humans at solving cybersecurity challenges, but they are both starting to score very high on multiple benchmarks. In this report, it is shown that the open source model \enquote{Mixtral} scores almost as good as GPT 4o in the image above. Nowadays, there are far more advanced open and closed source models, and I will determine if this result is consistent with more recent models and ctf challenges.
...
Second, the sample size, while including ten diverse models representing both open-source and closed-source approaches, remains relatively small for drawing definitive conclusions about the broader capabilities of each model category. The rapidly evolving nature of LLM development means that newer models, and unique models are constantly being developed. In order to get a true representation of open-source versus proprietary models, more models in both categories must be measured.
...
\subsection{Open-Source vs. Closed-Source Performance}

The results of this study reveal an interesting and somewhat unexpected pattern regarding the performance of open-source versus closed-source models. While closed-source models (Claude Opus 4.5 and Grok 4.1 Fast) successfully solved the challenge, the open-source model Kimi K2 Thinking also demonstrated successful problem-solving capabilities. This challenges the conventional wisdom that closed-source models uniformly outperform their open-source counterparts in complex reasoning tasks.

More significantly, our analysis of the reasoning processes revealed that open-source models often demonstrated comparable, and in some cases superior, reasoning approaches compared to closed-source models. The explicit thinking models, particularly those in the open-source category, provided detailed step-by-step reasoning that allowed for better understanding of their problem-solving strategies. This transparency is particularly valuable in cybersecurity applications where understanding the reasoning behind a conclusion is as important as the conclusion itself.

The fact that some open-source models produced incorrect solutions while showing strong reasoning capabilities suggests that the gap between open-source and closed-source models may be more about implementation, context, and prompt refinement. This is encouraging for organizations considering open-source alternatives for cybersecurity applications, as it indicates that with proper prompting, tools, and context, open-source models could potentially achieve parity with closed-source alternatives.
...
Third, research should explore the development of specialized cybersecurity-focused open-source models that leverage the transparency advantages of open-source architectures while incorporating domain-specific knowledge and reasoning patterns. The success of models like Kimi K2 Thinking suggests that explicit reasoning approaches can be particularly effective for complex cybersecurity challenges.

\subsection{Practical Recommendations}

Based on the findings of this study, organizations considering LLM deployment for cybersecurity applications should carefully evaluate both open-source and closed-source options based on their specific requirements. For applications where transparency and auditability are paramount, open-source models with explicit reasoning capabilities may be preferable despite potentially requiring more specific prompts and instructions to achieve a similar level of accuracy as proprietary models.

Organizations should also consider implementing MCP-based interfaces for their LLM deployments, as this standardization can improve interoperability with existing security tools and provide more consistent performance across different model types.

Finally, the rapid pace of LLM development means that organizations should establish processes for regular re-evaluation of model performance as new versions and architectures become available. The competitive landscape between open-source and closed-source models continues to evolve, and today's performance advantages may not persist in the rapidly changing AI ecosystem.

In conclusion, while this study demonstrates that both open-source and closed-source LLMs can effectively solve complex cybersecurity challenges, it also highlights the need for more comprehensive evaluation methodologies and the importance of standardized interfaces like MCP for future benchmark development. The gap between open-source and closed-source models appears to be narrowing, particularly in reasoning capabilities, suggesting that organizations have increasingly viable options for both approaches in their cybersecurity toolkits.
