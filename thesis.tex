...
The questions in HLE are developed by experts in their respective fields and vetted for clarity, correctness, and appropriate difficulty level. Each question is designed to test deep understanding and complex reasoning abilities that represent the current frontier of human academic achievement.
...
\textbf{SWE-bench Verified} is a human-validated subset of the original SWE-bench benchmark designed to more reliably evaluate AI models' ability to solve real-world software engineering problems \parencite{jimenez2024swebench}. The original SWE-bench consists of 2,294 software engineering problems drawn from real GitHub issues and corresponding pull requests across 12 popular Python repositories \parencite{jimenez2024swebench}.
...
\vspace{0.3em}
\footnotesize
\textbf{Reasoning:} Query the oracle repeatedly with the value $4$. Since $r^2 \equiv 4 \pmod{n}$, we have $n \mid (r^2 - 4)$. Taking $\gcd$ values from multiple such outputs recovers $n$. For factoring, if we obtain a non-trivial root $r$ satisfying $r \not\equiv \pm 2 \pmod{n}$, then $\gcd(r-2, n)$ or $\gcd(r+2, n)$ yields a factor. Once $p$ and $q$ are known, compute $d = e^{-1} \bmod \varphi(n)$ and then compute the signature $s = m^{d} \bmod n$.
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\textwidth}
\centering
...
\vspace{0.3em}
\footnotesize
\textbf{Reasoning:} Query the oracle with random values of $a^2$ to collect pairs $(a, y)$. Compute 
...
\vspace{0.3em}
\footnotesize
\textbf{Reasoning:} Query $f(1)$ multiple times to recover $n$. 
...
\vspace{0.3em}
\footnotesize
\textbf{Reasoning:} Incorrectly assumes that the square root of $m$ can serve as a valid signature. 
...
\vspace{0.3em}
\footnotesize
\textbf{Reasoning:} The model recognized that it must collect square roots, and correctly recognized it could add two roots $r_1$, and $r_2$ to get n, but then it gets confused and forgets that it knows how to solve for n.
...
\vspace{0.3em}
\footnotesize
\textbf{Reasoning:} Made critical error assuming n = y\(_0\) when querying f(0). Also incorrectly used d = inverse(e, n-1) instead of \(\phi\)(n) for computing the private exponent. The approach shows fundamental misunderstanding of RSA mathematics and the square root oracle's behavior.
\end{minipage}
...
\vspace{0.3em}
\footnotesize
\textbf{Reasoning:} Provided only a partial observation: 
...
\vspace{0.3em}
\footnotesize
\textbf{Reasoning:} The model struggled with the requirement that $\gcd(a-b, n)$ computations require knowing $n$. 
...
\vspace{0.3em}
\footnotesize
\textbf{Reasoning:} Incorrectly focused on the property $e = 2^{16} + 1$, assuming that taking 16 square roots would produce a valid signature. 
...
\vspace{0.3em}
\footnotesize
\textbf{Reasoning:} Understood the correct theory: recover $n$ using $\gcd(r^2 - x)$ values and factor $n$ using non-trivial square roots. 
